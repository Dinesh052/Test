...
walls. The environment is a **Constrained MDP**: the negotiator optimizes
de-escalation reward subject to safety constraints (supervisor), time
constraints (commander patience), and transparency constraints (media pressure).

Trained with **adversarial co-evolution** (GRPO) on Qwen 2.5-3B — the negotiator
and hostage-taker improve against each other over 4 rounds. The co-evolved 3B
model **nearly doubles zero-shot performance** (0.60 vs 0.33) and **matches
harm rate with the hand-coded FBI heuristic** (3%). Theory-of-Mind belief
